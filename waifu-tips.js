...
function initWidget(waifuPath, apiPath) {
	...
	$("body").append('<div id="waifu">\
			...
			<canvas id="live2d" width="220" height="330"></canvas>\
			<div id="waifu-tool">\
	...
	$("#waifu-tool .fa-camera-retro").click(function() {
 	...
 	$("#waifu-tool .fa-info-circle").click(function() {
 		window.open("https://www.devilcky.com/about/");
 	});
	...
	(function() {
		...
		if (location.href == SiteIndexUrl) { //如果是主页
			...
			else if (now > 7 && now <= 11) text = "上午好！要认真学习，不要久坐，多起来走动走动哦！";
			else if (now > 11 && now <= 14) text = "中午了，学习了一个上午，现在是午餐时间！";
			else if (now > 14 && now <= 17) text = "午后很容易犯困呢，今天定下的目标完成了吗？";
